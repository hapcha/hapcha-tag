...
encode(test_structure){const multiple_ingredients=(test_structure instanceof Array);const ingredients=multiple_ingredients?test_structure:[test_structure];const fields=this.decode_map.map(f=>f.field);let output=ingredients.map(ingredient=>{let ingredient_string="v1";Object.keys(ingredient).map(key=>{if(fields.indexOf(key)>=0){if(key!='version'){const flag=this.decode_map[fields.indexOf(key)].flag;const val=this.decode_map[fields.indexOf(key)].type=='string'?'"'+ingredient[key]+'"':ingredient[key];ingredient_string+=flag+val}}});return ingredient_string});if(output.length>1){output=output.map(x=>'['+x+']');output=output.join()}
return output}}